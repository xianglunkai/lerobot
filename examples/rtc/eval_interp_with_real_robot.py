...
class RTCDemoConfig(HubMixin):
    ...
    policy_fps: float = 30.0  # Action execution frequency (Hz)
    robot_fps: float = 50.0  # Robot control frequency (Hz)
...
def actor_control(
    ...
    try:
        ...
        action_count = 0
        action_interval = 1.0 / cfg.robot_fps # Robot control rate
        effective_policy_interval = 1.0 / cfg.policy_fps  # Action consume rate
        interpolator.reset()
        ...
        while not shutdown_event.is_set():
            ...
            if (start_time - last_action_consume_time) >= effective_policy_interval:
                action = action_queue.get()
                last_action_consume_time = start_time
            ...
            if interp_action is not None:
                action_dict = {key: interp_action[i].item() for i, key in enumerate(robot.action_features())}
                action_processed = robot_action_processor((action_dict, None))
                robot.send_action(action_processed)

                # Store executed action for visualization
                if viz_queue is not None:
                    viz_queue.put_nowait((time.time(), interp_action.clone()))
