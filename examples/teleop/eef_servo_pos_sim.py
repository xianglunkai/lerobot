#!/usr/bin/env python3
"""Position-level IK servo simulation (no ROS) – contrast with velocity-level version.

Uses the original AgilexCobot.send_action_from_eef logic:
    - accumulate delta into an absolute EEF pose
    - call inverse_kinematics every frame
    - directly use the returned joint positions as command

Usage:
    python eef_servo_gamepad_sim_ik.py [--history 900] [--save out.png]
"""
...
logger = logging.getLogger("eef_servo_gamepad_sim_ik")
...
class EefServoSimConfigIK:
    ...
    # Optional low-pass filtering on deltas (off by default for pure position IK)
    use_delta_filter: bool = False
    delta_filter_cutoff_hz: float = 10.0
...
class EefServoSimulatorIK:
    """Mirrors the position‑IK logic of AgilexCobot.send_action_from_eef."""

    def __init__(self, config: EefServoSimConfigIK, initial_q: list[float]):
        self.config = config
        ...
        # Current joint state (including gripper, 7 elements)
        self._q = np.array(initial_q, dtype=np.float64)
        # For filtered deltas (optional)
        self._filtered_delta = np.zeros(6, dtype=np.float64) if config.use_delta_filter else None
        # Previous EE position for velocity estimation (plotting)
    ...
    def step(self, ee_command: dict) -> dict:
        ...
        # Optional low‑pass filter on deltas (off by default)
        if self.config.use_delta_filter:
            raw_delta = np.array([delta_x, delta_y, delta_z,
                                  delta_roll, delta_pitch, delta_yaw], dtype=np.float64)
            fc = self.config.delta_filter_cutoff_hz
            alpha = np.clip(2.0 * np.pi * fc * dt, 0.0, 1.0)
            self._filtered_delta = (1.0 - alpha) * self._filtered_delta + alpha * raw_delta
            delta_x, delta_y, delta_z, delta_roll, delta_pitch, delta_yaw = self._filtered_delta

        # Current arm joints (exclude gripper)
        q_arm = self._q[:-1]

        # Compute current end‑effector pose
        current_ee = self.kinematics.forward_kinematics(q_arm)  # returns 4x4 in rad

        # Build desired pose by applying deltas
        ref = current_ee.copy()
        ...
        # IK: initial guess = current arm joints
        desired_q_arm = self.kinematics.inverse_kinematics(q_arm, desired)

        # Assemble full 7‑DOF command
        q_target = np.zeros(7, dtype=np.float64)
        q_target[:-1] = desired_q_arm
        ...
        # Estimate EE velocity for plotting (back‑difference of EE positions)
        if self._prev_ee_pos is not None:
            ee_vel_est = (current_ee[:3, 3] - self._prev_ee_pos) / dt
        else:
            ee_vel_est = np.zeros(3, dtype=np.float64)

        # Update state
        self._prev_ee_pos = current_ee[:3, 3].copy()
        prev_q = self._q.copy()
        self._q = q_target

        # Joint velocity (numerical diff)
        q_dot = (q_target[:-1] - prev_q[:-1]) / dt
...
def run_sim_ik(
    ...
    if agilex:
        sim_config = agilex_sim_config_ik()
        if eef_fps != 30.0:
            sim_config.eef_input_fps = eef_fps
        arm_q = initial_q or DEFAULT_AGILEX_ARM_Q
    else:
        sim_config = EefServoSimConfigIK(eef_input_fps=eef_fps)
    ...
    sim = EefServoSimulatorIK(sim_config, arm_q)
    ...
    controller = threading.Thread(target=control_loop, daemon=True, name="IKServoLoop")
    controller.start()

    # Plotting – same layout as velocity version
    fig, axes = plt.subplots(4, 1, figsize=(12, 10), sharex=True)
    delta_labels = ["delta_x", "delta_y", "delta_z"]
    ee_vel_labels = ["ee_vx", "ee_vy", "ee_vz"]
    ...
    lines_ee_vel = [axes[1].plot([], [], label=label)[0] for label in ee_vel_labels]
    ...
    fig.suptitle("Position‑IK EEF servo (no ROS) – Expect jitter when pressing D-pad")
    ...
    def update(_frame):
        ...
        if not data:
            return lines_delta + lines_ee_vel + lines_q + lines_qd
        ...
        for i, label in enumerate(ee_vel_labels):
            values = np.array([d[label] for _, d in data], dtype=float)
            lines_ee_vel[i].set_data(timestamps, values)
...
def main():
    parser = argparse.ArgumentParser(description="Position‑IK EEF servo simulation (no ROS)")
    ...
    parser.add_argument("--target-frame", type=str, default=None,
                        help="IK target frame (default: gripper_frame_link)")
    parser.add_argument("--ik-joint-names", nargs=7, metavar="JOINT", default=None,
                        help="Seven joint names: 6 arm joints + gripper")
    parser.add_argument("--eef-fps", type=float, default=30.0, help="Control loop rate (Hz)")
    parser.add_argument("--agilex", action="store_true",
                        help="Use AgilexCobotConfig URDF/joint names")
    ...
    try:
        ...
        logger.exception("Position‑IK EEF servo simulation failed")
