...
class QnbotWTeleop(Teleoperator):
    """
    QnbotW teleoperator that wraps two single-arm teleoperators into a bimanual system.
    
    Note: This implementation is provided for completeness. For ROS2-based QnbotW
    ...
    Example usage:
        ...
        qnbot_w_config = QnbotWTeleopConfig(
            id="qnbot_w_teleop",
            left_teleop=left_config,
            right_teleop=right_config
        )
        
        teleop = QnbotWTeleop(qnbot_w_config)
        teleop.connect()
        ```
    """
    
    config_class = QnbotWTeleopConfig
    name = "qnbot_w_teleop"
    
    def __init__(self, config: QnbotWTeleopConfig):
