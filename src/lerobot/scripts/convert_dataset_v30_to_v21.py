...
def convert_info(
    ...
    # 重命名 features 键：v3 -> v2，并将 v3 的 video 特征落到 v2.1 的 image 特征
    new_features: dict[str, Any] = {}
    for key, ft in info["features"].items():
        if key in V30_TO_V21_VIDEO_MAPPING:
            v2_key = V30_TO_V21_VIDEO_MAPPING[key][0]
        else:
            v2_key = key

        # copy to avoid mutating source dict from info
        ft = dict(ft)
        dtype = ft.get("dtype")

        # Normalize names layout to match legacy v2.1 json (names: [[...]] for vector features)
        names = ft.get("names")
        if isinstance(names, list) and names and all(isinstance(x, str) for x in names):
            ft["names"] = [names]

        if isinstance(dtype, str) and dtype.lower() == "video" and v2_key in video_keys:
            # v2.1 expects images as `image` feature (HF datasets Image => parquet struct {bytes, path})
            shape = ft.get("shape")
            height = width = channels = None
            if isinstance(shape, list) and len(shape) == 3:
                # v3.0 video feature often reports shape as [H, W, C]
                try:
                    height, width, channels = int(shape[0]), int(shape[1]), int(shape[2])
                except Exception:
                    height = width = channels = None
            info_dict = ft.get("info")
            if isinstance(info_dict, dict):
                height = height or info_dict.get("video.height")
                width = width or info_dict.get("video.width")
                channels = channels or info_dict.get("video.channels")

            if height is not None and width is not None:
                ft["shape"] = [3, int(height), int(width)]
            else:
                # fallback to legacy convention even if unknown
                ft["shape"] = [3, 0, 0]
            ...
            # Non-video features: legacy info.json should not carry fps
            ft.pop("fps", None)
...
def _decode_video_frames(video_path: Path, num_rows: int) -> list[np.ndarray | None]:
    """Decode video into RGB numpy arrays shape (H,W,3).
    返回长度为 num_rows 的列表，元素为 ndarray 或 None。若解码失败则全为 None。
    """
    ...
    if HAVE_DECORD:
        try:
            ...
            for i in range(min(length, num_rows)):
                img = vr[i].asnumpy()  # RGB uint8 HxWx3
                frames.append(img)
        except Exception:
            frames = []
    # 其次 OpenCV
...
def convert_data(
    ...
    for (chunk_idx, file_idx), records in tqdm.tqdm(grouped.items(), desc="convert data files"):
        ...
        for record in records:
            ...
            # 添加图像列：使用 v2.1 列名；存成 HF datasets Image 结构 (bytes/path)
            if video_keys:
                added_image_columns: list[str] = []
                for v2_col_name in video_keys:
                    if v2_col_name in episode_table.column_names:
                        continue
                    cam_dir = _extract_camera_name(v2_col_name)
                    ...
                    if not embed_images:
                        raise ValueError(
                            "v2.1 image features require embedding frames as PNG bytes (Image{bytes,path}). "
                            "Re-run with --embed-images (default) or implement PNG frame extraction to disk."
                        )

                    frame_list = _decode_video_frames(full_video_path, num_rows)
                    image_rows: list[dict[str, Any]] = []
                    for frame in frame_list:
                        if frame is None:
                            image_rows.append({"bytes": None, "path": None})
                            continue
                        # frame is RGB uint8; cv2 expects BGR for encoding
                        bgr = frame[..., ::-1]
                        ok, buf = cv2.imencode(".png", bgr)
                        if not ok:
                            image_rows.append({"bytes": None, "path": None})
                            continue
                        image_rows.append({"bytes": buf.tobytes(), "path": None})

                    image_type = pa.struct([
                        pa.field("bytes", pa.binary()),
                        pa.field("path", pa.string()),
                    ])
                    images_array = pa.array(image_rows, type=image_type)
                    episode_table = episode_table.append_column(v2_col_name, images_array)
                    added_image_columns.append(v2_col_name)

                if added_image_columns:
                    episode_table = _ensure_hf_image_features(episode_table, added_image_columns)
...
def convert_videos(root: Path, new_root: Path, episode_records: list[dict[str, Any]], video_keys_v3: list[str]) -> None:
    ...
    for v3_key in video_keys_v3:
        if v3_key in V30_TO_V21_VIDEO_MAPPING:
            v2_col, dest_dir = V30_TO_V21_VIDEO_MAPPING[v3_key]
        else:
...
def _ensure_hf_image_features(table: pa.Table, image_columns: list[str]) -> pa.Table:
    """Declare struct{bytes,path} columns as HF `Image` features via schema metadata."""
    ...
    if b"huggingface" in metadata:
        try:
            decoded = metadata[b"huggingface"].decode("utf-8")
            hf = json.loads(decoded) if decoded else {}
        except Exception:
            hf = {}
    else:
        hf = {}
    ...
    new_metadata = dict(metadata)
    new_metadata[b"huggingface"] = json.dumps(hf, separators=(",", ":")).encode("utf-8")
    return table.replace_schema_metadata(new_metadata)
...
def convert_dataset(
    ...
    video_keys_v2 = [
        V30_TO_V21_VIDEO_MAPPING[k][0] if k in V30_TO_V21_VIDEO_MAPPING else k
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="在每个 observation.images.* 列中嵌入 PNG bytes（默认开启，可能较慢）。",
    )
    parser.add_argument(
        "--no-embed-images",
        dest="embed_images",
        action="store_false",
        help="不嵌入图片（不推荐；v2.1 image 特征通常需要 bytes/path）。",
    )
